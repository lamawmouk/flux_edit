...
def main():
    ...
    parser.add_argument("--model", type=str, default="black-forest-labs/FLUX.1-dev", help="Model name or path")
    ...
    parser.add_argument("--start_timestep", type=float, default=0, help="Start timestep (as a fraction of total steps)")
    parser.add_argument("--stop_timestep", type=float, default=0.25, help="Stop timestep (as a fraction of total steps)")
    ...
    pipe = RFInversionFluxPipeline.from_pipe(pipe) 
    pipe = pipe.to(device)
    ...
    # Preprocess the input image to generate latents
    ...
    for i in range(args.num_images):
        ...
        # Call the pipeline with the generated latents
        result = pipe(
            prompt=args.prompt,
            prompt_2=prompt_2,
